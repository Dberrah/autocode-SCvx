...
class FirstOrderHold:
    ...
    def calculate_discretization(self, X, U):
        ...
        for k in range(self.K - 1):
            ...
            V = np.array(self.ode_euler(self._ode_dVdt, self.V0, (0, self.dt), U[:, k], U[:, k + 1]))
            ...
            # print(V[self.B_bar_ind].reshape((self.n_x, self.n_u)))
    ...
    def ode_euler(self, ode, V0, t, u_t0, u_t1):
        # Compute Euler explicite (ode solver)
        n = 150
        time = np.linspace(t[0],t[1],n)
        listeY = [V0]
        for k in range(1, len(time)) : # for k=1; k<n; k++
            dt = time[k] - time[k-1]
            yk = listeY[k-1] + ode(listeY[k-1], time[k], u_t0, u_t1)*dt
            listeY.append(yk)
        return listeY[-1]
    ...
    def _ode_dVdt(self, V, t, u_t0, u_t1):
        ...
        # print(V[self.A_bar_ind].reshape((self.n_x, self.n_x)))
    ...
    def integrate_nonlinear_piecewise(self, X_l, U):
        ...
        for k in range(self.K - 1):
            ...
            X_nl[:, k + 1] = self.ode_euler(self._dx, X_l[:, k], (0, self.dt), U[:, k], U[:, k + 1])
    ...
    def integrate_nonlinear_full(self, x0, U):
        ...
        for k in range(self.K - 1):
            ...
            X_nl[:, k + 1] = self.ode_euler(self._dx, X_nl[:, k], (0, self.dt), U[:, k], U[:, k + 1])
