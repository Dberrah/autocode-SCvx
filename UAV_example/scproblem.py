...
class SCProblem:
	...
	def __init__(self, m, K):
		# Variables:
		self.var = dict()
		self.var['X'] = cvx.Variable((m.n_x, K), name="X")
		self.var['U'] = cvx.Variable((m.n_u, K), name="U")
		self.var['nu'] = cvx.Variable((m.n_x, K - 1), name="nu")

		# Parameters:
		self.par = dict()
		self.par['A_bar'] = cvx.Parameter((m.n_x * m.n_x, K - 1), name="A_bar")
		self.par['B_bar'] = cvx.Parameter((m.n_x * m.n_u, K - 1), name="B_bar")
		self.par['C_bar'] = cvx.Parameter((m.n_x * m.n_u, K - 1), name="C_bar")
		self.par['z_bar'] = cvx.Parameter((m.n_x, K - 1), name="z_bar")

		self.par['X_last'] = cvx.Parameter((m.n_x, K), name="X_last")
		self.par['U_last'] = cvx.Parameter((m.n_u, K), name="U_last")

		self.par['weight_nu'] = cvx.Parameter(nonneg=True, name="weight_nu")
		self.par['tr_radius'] = cvx.Parameter(nonneg=True, name="tr_radius")
		self.par['tmp'] = cvx.Parameter((m.n_x, K*(len(m.obstacles)+len(m.obstacles_s)+len(m.obstacles_c))), name="tmp")

		# Constraints:
		constraints = []

		# Model:
		constraints += m.get_constraints(self.var['X'], self.var['U'], self.par['X_last'], self.par['U_last'], self.par['tmp'])
	...
	def get_parameter(self, name):
		...
		if name in self.par:
			return self.par[name].value
		else:
			print(f'Parameter \'{name}\' does not exist.')
			return None        
